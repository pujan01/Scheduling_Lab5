...
{ 
   ...
   */
	...
	// Initialize rem_bt i.e. making copy of plist with wt = 0
	for (i = 0; i < n; i++){
		rem_bt[i] = plist[i].bt;
		plist[i].wt = 0; 
	}
	
	int is_done = 0; //using it as flag
	while(!is_done){
		is_done = 1; 
		for(i = 0; i < n; i++){
			if (rem_bt[i] > 0){
				is_done = 0; 
				// Do the work for one quantum if rem_bt is greater than one quantum 
				if (rem_bt[i] > quantum){
					curr_time += quantum;
					rem_bt[i] -= quantum;
				}
				// Otherwise, set wt to curr_time - bt and increment process_counter  
				else{
					curr_time += rem_bt[i];
